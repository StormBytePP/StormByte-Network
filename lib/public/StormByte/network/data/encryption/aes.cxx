...
namespace {
	ExpectedCryptoBuffer EncryptHelper(std::span<const std::byte> dataSpan, const std::string& password) noexcept {
		try {
			...
			promise.set_value(StormByte::Util::Buffer(std::move(convertedData)));
...
ExpectedCryptoBuffer AES::Encrypt(const StormByte::Util::Buffer& input, const std::string& password) noexcept {
...
ExpectedCryptoBuffer AES::Encrypt(FutureBuffer& bufferFuture, const std::string& password) noexcept {
	try {
		StormByte::Util::Buffer buffer = bufferFuture.get();
...
ExpectedCryptoBuffer AES::Decrypt(const StormByte::Util::Buffer& input, const std::string& password) noexcept {
...
ExpectedCryptoString AES::RandomPassword(const size_t& passwordSize) noexcept {
	try {
		...
		return StormByte::Unexpected<CryptoException>(std::format("Failed to generate random password: {}", e.what()));
	}
}